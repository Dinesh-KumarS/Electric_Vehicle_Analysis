...
def dim_location(source_path):
    ...
    raw_location = raw_data \
            .select("Legislative District", "State", "County", "City", "Postal Code", "Vehicle Location") \
            .withColumnsRenamed({"Legislative District" : "Legislative_District", "Postal Code" : "Postal_Code", "Vehicle Location" : "Vehicle_Location"}) \
            .distinct()

    w = Window().orderBy("Legislative_District", "Vehicle_Location", "Postal_Code", "City", "County")
    cleaned_location = raw_location \
                .where("Vehicle_Location IS NOT NULL AND Legislative_District IS NOT NULL") \
                .withColumn("Geography_ID", row_number().over(w)) \
                .withColumns({"Postal_Code": col("Postal_Code").cast("Int"), "Legislative_District": col("Legislative_District").cast("Int")}) \
                .select("Geography_ID", "Legislative_District",  "State", "County", "City", "Postal_Code", "Vehicle_Location")

    logging.info("Saving data to Target File.!!")
    cleaned_location.write \
    .format("jdbc") \
